...
static void processObject( Obj *obj, Scene *scene, mmap& materials )
{
	...
	if( name == "directional_light" ) {
		...
		try {
			pointLight->constant_attenuation_coeff = getField(child, "constant_attenuation_coeff")->getScalar();
			pointLight->linear_attenuation_coeff = getField(child, "linear_attenuation_coeff")->getScalar();
			pointLight->quadratic_attenuation_coeff = getField(child, "quadratic_attenuation_coeff")->getScalar();
		} catch(...) {
		...
		scene->ambientLight = tupleToVec((getField(child, "color")));
	} else {
