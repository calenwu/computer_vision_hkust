...
public:
	...
	int GetAngle(const Point course);
	float getGrayValue(const Point course);

	void RightDown(const Point source, const Point target);
	void RightDrag(const Point source, const Point target);
	void RightUp(const Point source, const Point target);

	void SetLastCoordinateForAngle(const Point source);
