...
void Camera::lookAt(Vec3f eye, Vec3f at, Vec3f up) {
	Vec3f w(eye - at); // w = a = eye - at
	w.normalize(); // u = b x w, b = up vector
	Vec3f u = up ^ w;
	u.normalize();
	Vec3f v = w ^ u;
	u.normalize();
	float t0 = -u[0] * eye[0] - u[1] * eye[1] - u[2] * eye[2];
	float t1 = -v[0] * eye[0] - v[1] * eye[1] - v[2] * eye[2];
	float t2 = -w[0] * eye[0] - w[1] * eye[1] - w[2] * eye[2];
	Mat4f m = {
		u[0], u[1], u[2], t0,
		v[0], v[1], v[2], t1,
		w[0], w[1], w[2], t2,
